...
class lap_pyramid(nn.Module):
    """
    Normalised lapalcian pyramid implmeneted with filters learnt from
    the McGill natural image dataset.
    """
...
class lap_pyramid_GDN(nn.Module):
    """
    Trainable normalised laplacian pyramid with GDN modules to do the
    local divisive normalisation.
    """
    def __init__(self, k, dims=3, filt=None):
